...
class Handler {
    ...
    static async accountEdit(data) {
        if (data.method === "get") {
            try {
                // Get the outbound payload
                const payload = await AccountHandler.accountEdit();
                return {
                    statusCode : 200,
                    payload,
                    contentType : "text/html"
    ...
    static async accountDeleted(data) {
        if (data.method === "get") {
            try {
                // Get the outbound payload
                const payload = await AccountHandler.accountDeleted();
                return {
                    statusCode : 200,
                    payload,
                    contentType : "text/html"
     ...
     * @return {Promise<string | object>} - the outbound payload, either the html string upon success or error message in JSON format
     */
    static async checksCreate(data) {
        if (data.method === "get") {
            try {
                // Get the outbound payload
                const payload = await ChecksHandler.checksCreate();
                return {
                    statusCode : 200,
                    payload,
                    contentType : "text/html"
    ...
    /**
     * Checks Dashboard page handler
     * @param data - parsed request payload
     * @return {Promise<string | object>} - the outbound payload, either the html string upon success or error message in JSON format
     */
    static async checksList(data) {
        if (data.method === "get") {
            try {
                // Get the outbound payload
                const payload = await ChecksHandler.checksList();
                return {
                    statusCode : 200,
                    payload,
                    contentType : "text/html"
    ...
    /**
     * Edit/Delete Check page handler
     * @param data - parsed request payload
     * @return {Promise<string | object>} - the outbound payload, either the html string upon success or error message in JSON format
     */
    static async checksEdit(data) {
        if (data.method === "get") {
            try {
                // Get the outbound payload
                const payload = await ChecksHandler.checksEdit();
                return {
                    statusCode : 200,
                    payload,
                    contentType : "text/html"
