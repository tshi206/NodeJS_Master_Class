...
class Workers {
    ...
    constructor() {
        ...
        console.log("\x1b[33m%s\x1b[0m", "Workers status: on call")
    }
    ...
    static async gatherAllChecks() {
        ...
        if (result.length === 0) {
            debug("There is currently no checks launched in the system")
        }
        result.forEach(async check => {
            ...
            if (!validation) {
                debug(`Error : Check ID [ ${originalCheckData.id} ] of user [ ${originalCheckData.userPhone} ] is not properly formatted. Skipping it.`);
                return
            }
            ...
            if (typeof (newCheckData) === "string") {
                ...
                debug(await Workers.alertUserToStatusChanged(newCheckData))
            }
        })
    }
    ...
    loop() {
        const interval = setInterval(async () => {
            try {
                ...
                debug(e);
                console.log("Workers status: terminated with error");
